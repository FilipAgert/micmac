...
contains
    ...
    subroutine test_find_mindist(n_passed, n_failed)
        integer :: n_passed, n_failed, i
        real(kind) :: r, theta, theta_min, dist, expected, actual
        ...
        call conj_grad_method(theta_min,dist,converged,distfunc,0.0_kind,pi,theta,1e-5_kind)
    ...
    subroutine test_coul(n_passed, n_failed)
        ...
        vcoul%def = def
        vcoul%radius = 1.0_kind
        call vcoul%set_charge_dens(Z)


        actual = vcoul%eval(0.0_kind, 0.0_kind)
        expected = el_pot(0.0_kind, vcoul%radius, vcoul%charge_dens)
        ...
        actual = vcoul%eval(1.0_kind, 0.0_kind)
        expected = el_pot(1.0_kind, vcoul%radius, vcoul%charge_dens)
        
        pass = eq_r(expected, actual, 1e-1_kind)  !
        if (pass) then
            n_passed = n_passed + 1
        else
            n_failed = n_failed + 1
        ...
        actual = vcoul%eval(3.0_kind, 0.0_kind)
        expected = el_pot(3.0_kind, vcoul%radius, vcoul%charge_dens)
        pass = eq_r(expected, actual, 1e-4_kind)  !
        if (pass) then
            n_passed = n_passed + 1
        else
            n_failed = n_failed + 1
    ...
    subroutine testST(n_passed, n_failed)
        integer :: n_passed, n_failed
        real(kind) :: r, theta, theta_min, dist, expected, actual, val, eta, val2
        logical ::converged, pass
        ...
        s1%nz = 2
        s2%nz = 1
        actual = S0(1.5_kind,s1,s2)
        expected = S0(1.5_kind,s2,s1)
        ...
        actual = Sp(1.5_kind,s1,s2)

        expected = Sm(1.5_kind,s2,s1)
        pass = eq_r(expected, actual)  ! Replace with actual call: e.g. call micmac_result(actual)
        if (pass) then
            n_passed = n_passed + 1
        else
        ...
        eta = 1.0_kind
        expected = Tplus(eta,1,s1,s2,pot,1.0_kind,1.0_kind)
        actual = Tminus(eta,1,s2,s1,pot,1.0_kind,1.0_kind)
        pass = eq_r(expected, actual)  ! Replace with actual call: e.g. call micmac_result(actual)
        if (pass) then
            n_passed = n_passed + 1
        else
    ...
    subroutine test_deriv(n_passed, n_failed)
        real(kind) :: expected, actual, x
        real(kind), parameter :: dx = 1e-6_kind
    ...
    subroutine test_her(n_passed, n_failed)
        ...
        pass = eq_r(expected, actual,1e-2_kind)
        if (pass) then
            n_passed = n_passed + 1
        else
            n_failed = n_failed + 1
