module strutinsky
    ...
    real(kind), dimension(gauss_order) :: lag_x, lag_w, leg_x, leg_w
    contains
    ...
    real(kind) function microscopic_corrections(Z, A, def, fix_gamma) result(Emic_corr)
    ...
    real(kind) function shell_correction(Z, A, def, fix_gamma) result(Eshellcorr)
        integer, intent(in) :: Z, A
        real(kind), dimension(:), allocatable :: E_n, E_p
        type(betadef), intent(in) :: def
    ...
    subroutine purge_levels(levels, lim, N) !!purge levels lim above fermi level
        real(kind), dimension(:), allocatable, intent(inout) :: levels
        real(kind), dimension(size(levels)) :: temp
        real(kind), intent(in) :: lim
    ...
    function Jac_strut(gamma, efermi, levels, n_parts)
        ...
        real(kind), intent(in) :: gamma, efermi
        real(kind) :: Jac_strut(2,2)
        integer :: ll
        real(kind) :: el, tl
        Jac_strut = 0
        do ll = 1, size(levels)
    ...
    real(kind) function get_shell(gamma, efermi, n_parts, levels, hbaromega0, fix_gamma) result(E_shell) !gets shell correction energy
        integer, intent(in)::n_parts
        real(kind), intent(in) :: hbaromega0
        real(kind), dimension(:), intent(in) :: levels
        real(kind) :: E_sh, fermi_sh,  J(2,2), func(2), x(2), invJ(2,2), px(2)
        logical, intent(in) :: fix_gamma
        logical ::converged
        integer :: iter
        real(kind), intent(out) :: gamma, efermi
    ...
    function get_occ_numbers(levels, gamma, fermi) result(occ_numbers)
        real(kind), dimension(:), intent(in) :: levels
        real(kind), intent(in) :: gamma, fermi
        real(kind) :: u, E, t
        integer :: ii, numlevels, ll
        real(kind), dimension(size(levels)) :: occ_numbers

        call precompute_quad()
        numlevels = size(levels)
        occ_numbers = 0.0_kind
        do ll = 1, numlevels
            E = levels(ll)
            t = (fermi - E)/gamma
    ...
    pure elemental real(kind) function Peven(x,M)
        real(kind), intent(in) :: x
        integer, intent(in) :: M
        !!Even polynomial in the sturtinsky weighting integral
        Peven = lna(x*x,M,0.5_kind)
    end function

    pure elemental real(kind) function f_m(x,M)
        real(kind), intent(in) :: x
        integer, intent(in) :: M
        !!Even polynomial in the sturtinsky weighting integral
    ...
    real(kind) function smooth_e(levels, gamma, fermi) !!Energy of the smooth shell correction
        real(kind), dimension(:), intent(in) :: levels
        real(kind), intent(in) :: gamma, fermi
        real(kind) :: occ_nbr(size(levels))
    ...
    real(kind) function fermi_level_osc(num_parts, levels, gamma) result(fermi_osc)
        real(kind), dimension(:), intent(in) :: levels
        real(kind), intent(in) :: gamma
        integer, intent(in) :: num_parts
        real(kind) :: Ef_sh, lb, ub, arg, val, flb, fub
        integer :: status
        real(kind), parameter :: tol = 1e-3
    ...
    pure real(kind) function level_dens(E, levels, gamma)
        real(kind), intent(in) :: E, gamma, levels(:)
        integer :: ii
        real(kind) :: Eii
        level_dens = 0
        do ii = 1, size(levels)
