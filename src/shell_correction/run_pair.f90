program run_pairing
    ...
    real(kind), allocatable :: levels_p(:), levels_n(:), temp_p(:), temp_n(:)
    integer :: ii, Z, A, iostat, leastneg, numvals
    character(len=500) :: trash, line
    real(kind) :: ef, delta, G, I, E, smoothe, E0, epair, gamma, efp, efn, ldp, ldn, hbo, esh
    ...
    write(*,'(A,F10.3,A,F10.3)') "G:", G, ", 2/G:", 2.0_kind / G
    !call solve_BCS_eq(ef, delta, levels_p, Z, G)
    delta = 5.72/Z**(1.0_kind/3.0_kind) * exp(0.119*I -7.89*I**2)
    ...
    delta = 5.72/(A-Z)**(1.0_kind/3.0_kind) * exp(-0.119*I -7.89*I**2)
