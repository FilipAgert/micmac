module micmac
    ...
    type :: deformation 
        real(kind) :: alphas(2:(num_def_params+1)) = 0
    end type
    ...
    type, extends(func_nd) :: def_func_ho !!Class that creates a 1-dimensional function of binding energy as a function of deformation
        real(kind), private :: const, surf, colvol, econst, exp
    contains
...
subroutine setup_constants_nd(self, params, Z, A) 
    ...
    real(kind) :: av, as, k, r0, C, smallC, adef
    integer :: N
    av = params(1)
    as = params(2)
    k = params(3)
    ...
    self%exp  = -1.0_kind / (alpha0sqho(adef, r0, A))
end subroutine

pure real(kind) function calc_be_def_nd(self, xs)
...
pure function binding_energy_def(params, Z, A, def) result(BE)
    ...
    integer :: N
    real(kind) :: av, as, r0, kv, smallC, C, adef
    av = params(1)
    as = params(2)
    kv = params(3)
    r0 = params(4)
...
subroutine find_gs_multiple(BEs, defs, params, Zs, As, num_nuclei)
    real(kind), intent(out) :: BEs(num_nuclei)
...
subroutine find_gs(BE, def, found, params, Z, A)
    real(kind), intent(out) :: BE
    type(deformation), intent(out):: def
    logical, intent(out) :: found !!Flag for if found gs or not
    real(kind), intent(in) :: params(num_params)
    integer, intent(in) :: Z,A
    real(kind) :: defreal(num_def_params), lb(num_def_params), ub(num_def_params)
...
pure elemental real(kind) function volume_term(N, Z,av,kv)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: av,kv
    real(kind), parameter :: two_thirds = 2.0_kind/3.0_kind
    real(kind) :: I
    integer :: A
    A = N + Z
    I = (N-Z)*1.0_kind/real(A,kind=kind)
    volume_term = av*(1-kv*I*I)*A
end function volume_term

pure elemental real(kind) function surface_term(N, Z,as,ks)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: as,ks
    real(kind), parameter :: two_thirds = 2.0_kind/3.0_kind
    real(kind) :: I
    integer :: A
    A = N + Z
...
pure elemental real(kind) function col_term(N, Z, r0)
    implicit none
    integer, intent(in) :: Z, N
    real(kind), intent(in) :: r0
    integer :: A
    real(kind), parameter :: one_third = 1.0_kind/3.0_kind
    A = N + Z
    col_term = col_vol_term(N,Z,r0) + col_corr_term(N,Z,r0)
end function col_term

pure elemental real(kind) function col_vol_term(N,Z,r0)
    implicit none
    integer, intent(in) :: Z, N
    real(kind), intent(in) :: r0
    integer :: A
    real(kind), parameter :: one_third = 1.0_kind/3.0_kind
    A = N + Z
    col_vol_term = Z*Z *1.0_kind / (A**(one_third)) * 3.0_kind * e_squared / (5.0_kind * r0)


end function

pure elemental real(kind) function col_corr_term(N,Z,r0)
    implicit none
    integer, intent(in) :: Z, N
    real(kind), intent(in) :: r0
    integer :: A
    real(kind), parameter :: one_third = 1.0_kind/3.0_kind
    A = N + Z
    col_corr_term = - (Z * Z * 1.0_kind / A) * pi2/2 * (d/r0)**2 * e_squared/r0 

end function

pure elemental real(kind) function pairing_term(N, Z)
    implicit none
    integer, intent(in) :: N, Z
    integer :: A
    real(kind), parameter :: P = 11
    A = N + Z
    if(mod(A,2) == 1) then
        pairing_term = 0.0_kind
    else if(mod(N,2) == 1 .and. mod(Z,2) == 1) then
        pairing_term = P
    else
        pairing_term = -P
    end if
    pairing_term = pairing_term / sqrt(A*1.0_kind)
end function pairing_term




pure elemental real(kind) function shell_damp_fac_ho(N,Z, def, adef, r0) result(fac)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: adef, r0
    real(kind) :: alph0sq
    type(deformation), intent(in) :: def
    integer :: A
    A = N + Z
...
pure elemental real(kind) function def_f(def) result(f) !!10.1103/PhysRev.104.993
    type(deformation), intent(in) :: def
    real(kind) :: a2,a3,a4
    a2 = def%alphas(2)
    a3 = def%alphas(3)
    ...
    f = 1.0_kind + a2**2*(2.0_kind/5.0_kind) - a2**3 * 4.0_kind/105.0_kind - a2**4*(38.0_kind/175.0_kind) - a2**2*a4*(4.0_kind/35.0_kind)&
      +  a4**2  + a3**2 * ((5.0_kind/7.0_kind) - (8.0_kind/105.0_kind) * a2 - (18346.0_kind/13475.0_kind) * a2**2 - (4.0_kind/77.0_kind))
end function

pure elemental real(kind) function def_g(def) result(g) !!10.1103/PhysRev.104.993
    type(deformation), intent(in) :: def
    real(kind) :: a2,a3,a4
    a2 = def%alphas(2)
    a3 = def%alphas(3)
    ...
    g = 1.0_kind-(a2**2)/5.0_kind - a2**3 * 4.0_kind/105.0_kind + a2**4*(157.0_kind/1225.0_kind)&
      - a2**2*a4*(6.0_kind/35.0_kind) - a4**2*(5.0_kind/27.0_kind) &
      + a3**2 * (-(10.0_kind/49.0_kind) - a2*(92.0_kind/735.0_kind) &
      + (1701748.0_kind/3112725.0_kind) * a2**2 - (60.0_kind/539.0_kind)*a4)
end function
...
pure elemental function alpha0sqho(adef, r0, A)
    real(kind), intent(in) :: adef, r0
    integer, intent(in) :: A
    real(kind) :: alpha0sqho

    alpha0sqho = (adef/r0)**2 * A**(-2.0_kind/3.0_kind)
...
pure function J_mat(Zs, As, defs, num_nuclei, params)
    ...
    integer :: i, Z,A
    real(kind) :: J_mat(num_nuclei,num_params)
    type(deformation) :: def
...
pure function J_vec(Z,A,params, def)
    ...
    real(kind),intent(in) :: params(num_params)
    type(deformation), intent(in):: def
    real(kind) :: J_vec(num_params)
    real(kind) :: a0, av, as, r0, kv, smallC, C, adef
    av = params(1)
    as = params(2)
    kv = params(3)
    r0 = params(4)
...
pure real(kind) function dBe_da0()
    dBe_da0 = 1.0_kind
end function



pure elemental real(kind) function dBe_daV(N,Z,k)
    integer, intent(in) :: N, Z
    ...
    A = N + Z
    I = (N-Z)*1.0_kind/A
    dBe_daV = 1.0_kind-k * I*I
    dBe_daV = dBe_daV * A
end function

pure elemental real(kind) function dBe_daS(N,Z,k, def)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: k
    type(deformation), intent(in):: def
    real(kind) :: I
    ...
    A = N + Z
    I = (N-Z)*1.0_kind/A
    dBe_daS = 1.0_kind-k * I*I
    dBe_daS = dBe_daS * A**(2.0_kind/3.0_kind) * def_f(def)
end function

pure elemental real(kind) function dBe_dkv(N,Z,av, as, def)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: av, as
    type(deformation), intent(in):: def
    real(kind) :: I
    integer :: A
    A = N + Z
    I = (N-Z)*1.0_kind/A
    dBe_dkv = -I*I*(av*A) -I*I*(def_f(def)*as*A**(2.0_kind/3.0_kind))
end function


pure elemental real(kind) function dBe_dr0(N,Z,r0, def, C, smallC, adef)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: r0,  c, smallC, adef
    type(deformation), intent(in):: def
    integer :: A
    A = N + Z
    !!Coulomb terms where one term has macroscopic deformation 
    dBe_dr0 = 3.0_kind*e_squared*Z*Z*(5.0_kind*pi2*d*d-2*A**(2.0_kind/3.0_kind)*r0*r0*def_g(def))/(10.0_kind*A*r0**4)


    !!Shell correction term as alpha0 has r0
    dBe_dr0 = dBe_dr0 - 2.0_kind * r0*eff_def_sq(def)/(adef)**2 * A**(2.0_kind/3.0_kind) * shell_corr(N,Z,C,smallC)*shell_damp_fac_ho(N,Z,def,adef,r0) 
end function dBe_dr0
!!Partial derivate with respect to small C
pure elemental real(kind) function dBe_dsc(N,Z,C, def, adef, r0)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: C, adef, r0
    type(deformation), intent(in):: def
    integer :: A
    A = Z + N
    dBe_dsc = -C* A**(1.0_kind/3.0_kind) * shell_damp_fac_ho(N,Z,def,adef,r0)
end function dBe_dsc

!!Partial derivate of shell correction with respect to C
real(kind) elemental function dBe_dC(N,Z,smallC, def, adef, r0)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: smallC, adef, r0
    type(deformation), intent(in):: def
    integer :: A
    A = Z + N
    dBe_dC = (F(Z,.true.) + F(N,.false.)) / ((A*1.0_kind/2.0_kind)**(2.0_kind/3.0_kind)) - smallC * A**(1.0_kind/3.0_kind)
    dBe_dC = dBe_dC * shell_damp_fac_ho(N,Z,def,adef,r0)

end function dBe_dC

!!Partial derivate of shell correction with respect to C
pure real(kind) elemental function dBe_dadef(N,Z,C, smallC, def, adef, r0)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: smallC, adef, r0, C
    type(deformation), intent(in):: def
    integer :: A
    real(kind) :: sq
    sq = eff_def_sq(def)
    A = Z + N
    dBe_dadef = 2.0_kind * A**(2.0_kind/3.0_kind) * sq * r0**2/ (adef**3) * shell_corr(N,Z,C,smallC) * shell_damp_fac_ho(N,Z,def,adef,r0)
end function dBe_dadef

!! ########################## Shell correction Part
pure elemental real(kind) function shell_corr(N,Z, C, smallC)
    integer, intent(in) :: N, Z
    real(kind), intent(in) :: C, smallC
    integer :: A
    A = N + Z
    shell_corr = (F(Z,.true.) + F(N,.false.)) / ((A*1.0_kind/2.0_kind)**(2.0_kind/3.0_kind)) - smallC * A**(1.0_kind/3.0_kind)
    shell_corr = shell_corr * C
end function

pure elemental real(kind) function Eshell(A,Z,C,smallC, r0, adef, def)
    integer, intent(in) :: A, Z
    real(kind), intent(in) :: C, smallC, r0, adef
    type(deformation), intent(in):: def
    integer :: N
    N = A-Z
...
pure elemental real(kind) function F(N,proton)
    ...
    F = staircase(real(N,kind), magics)*(N-magic) - 3.0_kind/5.0_kind * (N**(5.0_kind/3.0_kind) - magic**(5.0_kind/3.0_kind))
end function

pure real(kind) function staircase(n, magics) !!Gets staircase function at value. If value exactly equals a magic number, use that as the lower bound
    real(kind), intent(in) :: n
    ...
    staircase = 3.0_kind/5.0_kind * (num_gr**(5.0_kind/3.0_kind) - num_l**(5.0_kind/3.0_kind)) / (num_gr-num_l)
end function


pure elemental real(kind) function eff_def_sq(def) result(a)
    type(deformation), intent(in) :: def
    !!Calculate (deltaR)^2/R0
    integer :: n
    a = 0.0_kind
    do n = 2, num_def_params
        a = a + def%alphas(n)**2 / real(2*n+1,kind)
    end do
end function
